...
bool doEdgesCross(const Edge &e1, const Edge &e2)
{
  if (e1.p != e2.p)
    return false;
 ...
 */
int countEdgeCrossings(const Graph &gr, const Edge &ed)
{
  int result = 0;
 ...
 */
int countEdgesFromVertexCrossings(const Graph &gr, const Vertex &ver)
{
  int result = 0;
 ...
 */
int countCrossingNumber(const Graph &gr)
{
  int result = 0;
 ...
 */
int countEdgesFromNeighborsCrossings(const Graph &gr, int v1)
{
  int result = 0;
 ...
 */
int countEdgesVertexCrossingsImpl(const Graph &gr, int v1, vector<Edge> &eList,
                                  int factor)
{
  int result = 0;
  const Vertex &ver1 = gr.v[v1];
...
int removeEdgesVertexCrossings(const Graph &gr, int v1, vector<Edge> &eList)
{
  return countEdgesVertexCrossingsImpl(gr, v1, eList, -1);
}

int addEdgesVertexCrossings(const Graph &gr, int v1, vector<Edge> &eList)
{
  return countEdgesVertexCrossingsImpl(gr, v1, eList, 1);
}

int addEdgesGraphCrossings(const Graph &gr, vector<Edge> &eList)
{
  int result = 0;
  for (Edge &ed1 : eList)
 ...
 */
void moveVertex(Graph *gr, int vOld, int vNew)
{
 ...
 */
void swapVertices(Graph *gr, int vA, int vB)
{
...
bool greedyEdgePage(Graph *gr, Edge *ed)
{
  bool improved = false;
...
void greedyAtVertex(Graph *gr, int v)
{
  for (Edge *ed : gr->v[v].neighs)
    greedyEdgePage(gr, ed);
}

void greedyPages(Graph *gr)
{
  bool improved = true;
  while (improved)
  {
 ...
 */
int findBestPositionForVertex(const Graph &gr, int origPos, int *finalPos)
{
  Graph mGr = Graph(gr);
...
int restartEdges(Graph *gr, int prevCr, void (*placer)(Graph *gr))
{
  Graph grBck = *gr;
  for (Edge &e : gr->e)
    e.p = -1;
...
int simAnneal(Graph *gr, double t0)
{
//  double t = t0;
  double t1 = 0.2;
  int endIter = 1000;
...
void verifyGraphBadCase(string msg)
{
  cerr << "The graph is bad!!! " << msg << endl;
  exit(0);
}

void verifyGraph(int claimedCr, const Graph &origGr, const Graph &gr)
{
...
void writeGraphToFile(int bestVal, const Graph &origGr, const Graph &bestGr,
                      string filename)
{
